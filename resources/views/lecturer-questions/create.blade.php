...
@section('content')
    ...
    <p class="mt-1 text-sm text-ink/60">Ketua yang meneruskan ke dosen. Jangan kirim sendiri ke chat dosen.</p>
    ...
    <form method="POST" action="{{ route('lecturer-questions.store') }}" class="mt-6 max-w-xl space-y-4 rounded-2xl border border-line bg-card p-6">
        ...
        <label class="flex items-start gap-2 text-sm text-ink/70">
            ...
            <span>Jangan sebut nama saya ke dosen. Ketua tetap tahu siapa yang menulis.</span>
        </label>
        <x-btn type="submit">Kirim ke ketua</x-btn>
    </form>
@endsection
